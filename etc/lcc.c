 ...
 */
static char rcsid[] = "$Id: lcc.c,v 1.1.1.1 2004/03/24 04:37:35 sjp Exp $";
...
int main(int argc, char *argv[]) {
	...
	/*                                                    */
	/* Check if /temp has been defined in the environment */
	/*                                                    */
	if (getenv("TMP"))
		tempdir = getenv("TMP");
	else if (getenv("TEMP"))
...
/* remove suffix, e.g. /usr/drh/foo.c => /usr/drh/foo */
char *removesuffix(char *name) {
	char *s, *b, *t = 0;
...
static int filename(char *name, char *base) {
	...
	case 1:	/* preprocessed source files */
		...
		else if ((status = compile(name, (stemp=tempname(first(suffixes[2]))))) == 0)
