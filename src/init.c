...
static char rcsid[] = "$Id: init.c,v 1.1.1.1 2004/03/24 04:37:33 sjp Exp $";
...
static int initchar(int len, Type ty) {
	...
	printf(";entered initchar\n");
	printf(";inttype->size = %d\n",inttype->size);
	...
	printf(";before last defstring \n");
	if (s > buf)
		(*IR->defstring)(s - buf, buf);
	printf(";exiting initchar\n");
	return n;
}
*/
...
Type initializer(Type ty, int lev) {
	...
	//print(";lc3 aty->op=%d\n",aty->op);
	//print(";lc3 CHAR=%d\n",CHAR);
	//print(";lc3 unqual(aty)->op=%d\n",unqual(aty)->op);
	//print(";lc3 ischar((aty))=%d\n",ischar(aty));
	//print(";lc3 isint((aty))=%d\n",isint(aty));
	//print(";lc3 isarray(ty->type)=%d\n",isarray(ty));
	...
	if (isarray(ty) && ischar(aty)) {
		...
		//print(";lc3 isarray but not ischar\n");
